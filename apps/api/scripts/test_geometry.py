...
from app.schemas.model import Difficulty, PaperSize, Style
...
def test_seam_split_limits() -> None:
    ...
    print("seam split easy:", len(patches), "patches", [len(p) for p in patches])


def test_unfold_and_layout() -> None:
    mesh = _box_mesh()
    data = compute_edge_dihedral_angles(mesh)
    ...
    print("unfold/layout:", len(pieces), "pieces", len(pages), "pages", "warnings:", warnings)


def main() -> None:
    test_signed_dihedral_box()
    test_seam_split_limits()
    test_unfold_and_layout()
